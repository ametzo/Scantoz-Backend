...
module.exports = {
    ...
    createDocument: async (file, req, res) => {
        try {
            const { generatedInvoice } = req.body;

            const parsedGeneratedInvoice = generatedInvoice
                ? JSON.parse(generatedInvoice)
                : [];

            console.log(parsedGeneratedInvoice, "generated");
            ...
            if (
                ...
                const { imagePath, imageSize, fileName } = await processFile(
                    file
                );

                const newGeneratedDetailData = {
                    imagePath,
                    imageSize,
                    fileName,
                    status: "pending",
                ...
                if (newDocument.documentType === "sales-invoice") {
                    Object.assign(newGeneratedDetailData, {
                        companyName,
                        grossAmount,
                        vatAmount,
                        totalAmount,
                ...
                const newGeneratedDetail = await GeneratedDetail.create(
                    newGeneratedDetailData
                );
                await newGeneratedDetail.save();
            }
    ...
    deleteDocument: async (req, res) => {
        try {
            ...
            await GeneratedDetail.updateMany(
                { documentId: id, isDeleted: false },
                { $set: { isDeleted: true } }
            );
    ...
    getAllDocuments: async (req, res) => {
        try {
            ...
            if (filterBy && (fromDate || toDate)) {
                ...
                //         ];
                //     } else if (fromDate) {
                //         filters["voucherAmendment.checkOutDate"] = {
                //             $gte: new Date(fromDate),
                //         };
                //     } else if (toDate) {
                //         filters["voucherAmendment.checkOutDate"] = {
    ...
    getSingleDocument: async (req, res) => {
        try {
            ...
            const invoices = await GeneratedDetail.find({
                $and: [
                    { documentId: document._id, isDeleted: false },
                    { ...filters2 },
                ],
            });
